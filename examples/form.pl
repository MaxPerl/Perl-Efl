...
sub _create_contact_list {
	...
	my $list = Efl::Elm::Genlist->add($parent);
	$list->show();
...
sub _create_contact_form {
	...
	my $vbox = Efl::Elm::Box->add($parent);
	$vbox->align_set(0,0);
	$vbox->show();
	
	# Add the icon to the vbox
	my $ic = Efl::Elm::Icon->add($vbox);
	$ic->file_set("./" . $contact->{icon}, undef );
	...
	foreach my $form_item (@form_items) {
		...
		# label
		my $label = Efl::Elm::Label->add($hbox);
		...
		# edit
		my $edit = Efl::Elm::Entry->add($hbox);
