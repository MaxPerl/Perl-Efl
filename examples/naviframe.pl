...
my $win = Efl::Elm::Win->util_standard_add("Naviframe", "Naviframe Tutorial");
$win->autodel_set(1);
...
sub _naviframe_add {
	...
	my $z = {};
	$z->{naviframe} = Efl::Elm::Naviframe->add($parent);
...
sub _naviframe_prev {
	...
	if ( ${$z->{naviframe}->bottom_item_get()} != ${$z->{naviframe}->top_item_get()} ) {
		...
		# If you use the Efl-Elm standard version you must bless the returned widget manually
		# as following:
		# my $content = $z->{naviframe}->item_pop();
		# bless($content, "ElmLabelPtr"); # or bless($content, "Efl::Elm::Label");
...
sub _button {
	...
	my $button = Efl::Elm::Button->add($z->{naviframe});
...
sub _naviframe_populate_gen {
	...
	my $i;
	for ($i = 0; $i < 20; $i++) {
		my $label = Efl::Elm::Label->add($z->{naviframe});
		my $label2 = Efl::Elm::Label->add($z->{naviframe});
