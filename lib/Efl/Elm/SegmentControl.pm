package Efl::Elm::SegmentControl;
...
require XSLoader;
XSLoader::load('Efl::Elm::SegmentControl');
...
For instructions, how to use Efl::Elm::SegmentControl, please study this API reference for now. A perl-specific documentation will perhaps come in later versions. But applying the C documentation should be no problem. Efl::Elm::SegmentControl gives you a nice object-oriented interface that is kept close to the C API. Please note, that the perl method names remove the "elm_segment_control_" at the beginning of the c functions.
